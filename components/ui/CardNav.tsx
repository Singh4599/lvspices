  ...
  return (
    <div className={`card-nav-container ${className}`}>
      <nav ref={navRef} className={`card-nav ${isExpanded ? 'open' : ''}`} style={{ backgroundColor: baseColor }}>
        ...
        <div className="card-nav-content" aria-hidden={!isExpanded} data-lenis-prevent="true">
          {(items || []).slice(0, 3).map((item, idx) => (
            ...
            >
              {item.icon && <div className="nav-card-icon">{item.icon}</div>}
              <div className="nav-card-label-wrap">
                <div className="nav-card-label">{item.label}</div>
                <div className="nav-card-underline" />
              </div>
