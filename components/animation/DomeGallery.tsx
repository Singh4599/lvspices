...
const DEFAULT_IMAGES: ImageItem[] = [
  { src: '/images/hero-spices.png', alt: 'Premium Spices' },
  { src: '/images/products.png', alt: 'Full Range' },
  { src: '/images/farm.png', alt: 'Farm Sourced' },
  { src: '/images/factory.png', alt: 'Manufactured' },
  { src: '/images/lab.png', alt: 'Lab Tested' },
  { src: '/images/cryo-dark.png', alt: 'Cryogenic' },
  { src: '/images/farm-editorial.png', alt: 'Field Fresh' },
  { src: '/images/cfg-bg.png', alt: 'CFG Science' },
];
...
export default function DomeGallery({
  ...
  // ── Auto-rotation ──────────────────────────────────────────────
  const autoRotRAF = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      // On narrow screens, cap minRadius so the dome doesn't overflow the viewport
      const effectiveMinRadius = w <= 768 ? Math.min(minRadius, w * 0.75) : minRadius;
  ...
  const stopAutoRotate = useCallback(() => {
    if (autoRotRAF.current) { cancelAnimationFrame(autoRotRAF.current); autoRotRAF.current = null; }
  }, []);
  ...
  const startAutoRotate = useCallback(() => {
    ...
    const tick = () => {
      if (draggingRef.current || inertiaRAF.current) {
        // suspend while user is interacting; restart after a short delay
        autoRotRAF.current = requestAnimationFrame(tick);
        return;
      }
  ...
  // Kick off auto-rotation on mount, stop on unmount
  useEffect(() => {
    startAutoRotate();
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    const vSx = isFinite(sx0) && sx0 > 0 ? sx0 : 1; const vSy = isFinite(sy0) && sy0 > 0 ? sy0 : 1;
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      animatingOverlay.addEventListener('transitionend', () => {
        ...
        parent.style.setProperty('--rot-y-delta', '0deg'); parent.style.setProperty('--rot-x-delta', '0deg');
        requestAnimationFrame(() => {
          el.style.visibility = ''; el.style.opacity = '0'; focusedElRef.current = null;
